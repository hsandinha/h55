...
export function ContactModal({
  ...
  const [form, setForm] = useState({
    ...
    telefone: "",
    email: "",
    mensagem: imovel ? `Tenho interesse no imóvel "${imovel.titulo}".` : "",
  });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const r = await createLead({
      ...
      origem: imovel ? "Site - Imóvel" : "Site - Contato",
    });
    setSaving(false);
    if (r.ok) setDone(true);
    else setError(r.error || "Não foi possível enviar. Tente novamente.");
  };
        ...
        {done ? (
          ...
          <form onSubmit={submit} className="space-y-4 p-6">
            ...
            <div>
              ...
              <textarea
                className="min-h-24 w-full border border-[#b8860b]/25 bg-white p-3 text-sm text-[#0a2540] placeholder-[#aab4c2] focus:border-[#b8860b] focus:outline-none"
                value={form.mensagem}
